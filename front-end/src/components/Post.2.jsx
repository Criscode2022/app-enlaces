    ...
    fetch(`http://localhost:3000/posts/likes`, {
      ...
      .then((data) => {
        ...
        const userLikedPost = data.likes.some((like) => {
          return (
            like.id_user === authenticatedUserId && like.id_post === postId
          );
        });
  ...
  return (
    ...
    >
      ...
      <CardContent>
        ...
        import React from 'react'; import PropTypes from 'prop-types'; function
        Enlace({onDelete}) {}
        const esCreador = enlace.creador === usuarioActual.id; const
        handleEliminarClick = () =&gt;{" "}
        {
          // Agregar la lógica para eliminar el enlace aquí
          onDelete(enlace.id)
        }
        ; &rbrace;; return (
        <div>
          <p>Título del Enlace: {enlace.titulo}</p>
          <p>URL: {enlace.url}</p>
          {esCreador && (
            <button onClick={handleEliminarClick}>Eliminar Enlace</button>
          )}
        </div>
        ); {"}"} Enlace.propTypes = {enlace}: PropTypes.shape({id}:
        PropTypes.number.isRequired, titulo: PropTypes.string.isRequired, url:
        PropTypes.string.isRequired, creador: PropTypes.number.isRequired,
        {"}"}).isRequired, usuarioActual: PropTypes.shape({id}:
        PropTypes.number.isRequired, {"}"}).isRequired, onDelete:
        PropTypes.func.isRequired, {"}"}; export default Enlace;
      </CardContent>
    </Card>
  );
};
